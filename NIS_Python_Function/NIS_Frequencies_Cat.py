def NIS_Frequencies_SubGroup(df,CodeSet,vv,Study_VV):
  ...
  ### Category Frequency ###
  category_columns = [item for item in Study_VV if item not in ["KEY_NIS", "DISCWT"]]
  my_dict = {}
  ...
  ### Writing File out ###
  with pd.ExcelWriter(Output_Path+Official+"SubGroup"+str(Year)+'.xlsx') as writer:
    for cccc in category_columns:
